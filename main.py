...
token = spotipy.util.prompt_for_user_token(USERNAME, SCOPE, client_id=SPOTIPY_CLIENT_ID,
...
def addSongsToGooglePlaylistFromInfo(playlist_id, track):
    ...
    # Looks for matching track in Google Play library
    for track in library:
        if matchTitle(track["album"], albumName) and matchTitle(track["albumArtist"], artistName) and matchTitle(
...
def addSongsToYtPlaylistFromInfo(playlist_id, track):
    ...
    for track in library:
        try:
            if matchTitle(track["album"]["name"], albumName) and matchTitle(track["artist"][0]["name"],
                                                                            artistName) and matchTitle(track["title"],
                                                                                                       trackName):
...
def ytToSpotify():
    ...
    print(
        "Note that if multiple playlists exist with the same name, the first one will be selected; rename the playlist to be unique to avoid this.")
    ...
    if targetPlaylist == None:
        ...
        spotifyPlaylist = spotify.user_playlist_create(USERNAME, playlistName, public=public,
        ...
        if failures:  # output any tracks that could not be found
            print(
                "Certain tracks could not be found on Spotify (these tracks may be unavailable, or have metadata that does not match YouTube's):")
            ...
            spotify.user_playlist_change_details(USERNAME, spotifyPlaylist["id"],
                                                 description=new_description[:300])  # 300 character limit
...
def spotifyToGoogle():
    ...
    if targetPlaylist == None:
        ...
        for i in playlistItems:
            if not addSongsToGooglePlaylistFromInfo(playlist_id, i):
                failures.append(i)
...
def googleToSpotify():
    ...
    if targetPlaylist == None:
        ...
        if failures:  # output any tracks that could not be found
            print(
                "Certain tracks could not be found on Spotify (these tracks may be unavailable, or have metadata that does not match Google's):")
            missing_tracks = ["Missing from original playlist:"]
            for failure in failures:
                track = failure[0] + " - " + failure[1] + " - " + failure[2]
                print(track)
